...
<div class="content">
    ...
    <!-- Vertical form options -->
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route('blog.update', [ $blog ->id]) }}" id="form-entry">
                        <input type="hidden" name="_method" value="PATCH">
                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                        <div class="form-group">
                            <label>Title Of The Blog</label>
                            <input type="text" class="form-control font-weight-light" placeholder="Enter Title" id="title" name="title" required  value="{{ $blog ->title  }}" >
                        </div>
                        <div class="form-group ">
                            <label class>Short Description <span class="text-danger" >*</span></label>
                            <div class>
                                <textarea rows="5" cols="5" id="description" name="description" class="form-control" required="" placeholder="Enter Description">{{   $blog ->description   }}</textarea>
                            </div>
                        </div>  
                        <label class>Blog Post <span class="text-danger">*</span></label>
                        <div class="form-group">
                            <textarea id="mysummernote" name="mysummernote" required   >{{ $blog ->mysummernote  }}</textarea>
